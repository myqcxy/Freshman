...
{
	int date;//数据元素类型
	struct stu *next;//下一个地址的指针
}LNode,*LinkList;
LinkList IntList(LinkList head);//生成链表并返回其地址
void ListInsert(LinkList head);//单链表的插入
void ListDelete(LinkList head);//单链表的删除
void LocateElem(LinkList head);//单链表中元素的定位
void UpdateElem(LinkList head);//修改但链表中的元素
void PrintLink(LinkList head);//输出单链表中的元素
void LinkRev(LinkList head);//将单链表中的元素逆置
int main()
{
	LinkList head;
	head=IntList(head);
